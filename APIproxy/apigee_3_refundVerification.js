...
//console.log("partnerJWTToken is: " + partnerJWTToken);
    var refundVerification_Response = http.post(
      `${env}/biller/v1/refund/verification`,
      JSON.stringify(body),
      { headers: {
    ...
    var result = JSON.parse(refundVerification_Response.body);
    //console.log("---------------response is:      " + result.responseCode);
    check(refundVerification_Response, {
...
export function handleSummary(data) {
  return {
    "summary_apigee_refundVerification.html": htmlReport(data),
  };
}