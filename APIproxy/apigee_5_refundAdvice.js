...
//requestTime = "10:21:50";//requestTime = `${HH}`+ ':' + `${minutes}` + ':' + `${ss}`;
...
//console.log("partnerJWTToken is: " + partnerJWTToken);
    var refundAdvice_Response = http.post(
      `${env}/biller/v1/refund/advice`,
      JSON.stringify(body),
      { headers: {
    ...
    var result = JSON.parse(refundAdvice_Response.body);
   // console.log("---------------response is:      " + result.responseCode);
    check(refundAdvice_Response, {
...
export function handleSummary(data) {
  return {
    "summary_apigee_refundAdvice.html": htmlReport(data),
  };
}